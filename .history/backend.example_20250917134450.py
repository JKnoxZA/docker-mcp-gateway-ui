...
"""
    ...
    def _generate_readme_tools(self, tools: List[Dict[str, str]]) -> str:
        ...
        tool_docs = []
        for tool in tools:
            tool_docs.append(f"- **{tool.get('name', 'Unnamed Tool')}**: {tool.get('description', 'No description provided')}")
        return "\n".join(tool_docs)
    ...
    def _generate_yaml_tools(self, tools: List[Dict[str, str]]) -> str:
        ...
        for tool in tools:
            ...
            yaml_tools.append(f"      description: {tool.get('description', 'No description')}")
        return "\n".join(yaml_tools)
    ...
    def update_registry_yaml(self, project_name: str):
        ...
        new_entry = {
            'ref': project_name,
            'url': f"docker://{project_name}:latest"
        }
        ...
        try:
            ...
            if containers:
                ...
                container = docker_client.containers.run(
                    ...
                    ports={"8811/tcp": 8811},
                    volumes={
                        str(self.config_root): {"bind": "/config", "mode": "rw"}
                    },
                    detach=True
                )
...
async def websocket_build_logs(websocket: WebSocket, build_id: str):
    ...
    await websocket.send_json({
        ...
        "message": f"Build {build_status.status}"
    })
...
async def test_mcp_server(server_name: str):
    ...
    try:
        ...
        return {"status": "success", "message": f"Successfully connected to {server_name}"}
    except Exception as e:
        server.status = "error"
...
async def handle_tool_permission(permission: ToolPermission):
    ...
    return {"message": "Permission recorded", "permission_id": len(tool_permissions) - 1}
...
async def get_gateway_status():
    ...
    try:
        ...
        return {
            ...
            "active_clients": len([c for c in llm_clients.values() if c.status == "connected"])
        }
...
async def execute_tool(tool_name: str, server_name: str, client_name: str, parameters: dict = {}):
    """Execute a tool from an MCP server (requires permission)"""
    
    # Check if tool execution is allowed
    has_permission = any(
        p.tool_name == tool_name and 
        p.server_name == server_name and 
        p.client_name == client_name and 
    ...
    try:
        while True:
            ...
            status_update = {
                ...
                "servers_connected": len([s for s in mcp_servers.values() if s.status == "connected"]),
                "clients_active": len([c for c in llm_clients.values() if c.status == "connected"]),
                "pending_permissions": len([p for p in tool_permissions if p.permission == "pending"])
            }
