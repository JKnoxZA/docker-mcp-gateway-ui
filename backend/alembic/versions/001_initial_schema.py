...
def upgrade() -> None:
    ...
    op.create_table('mcp_projects',
        ...
        sa.Column('status', sa.Enum('CREATED', 'BUILDING', 'BUILD_FAILED', 'BUILT', 'DEPLOYED', 'DEPLOY_FAILED', name='projectstatusenum'), nullable=True),
    ...
    op.create_index(op.f('ix_mcp_projects_name'), 'mcp_projects', ['name'], unique=False)
    ...
    op.create_table('mcp_servers',
        ...
        sa.Column('transport', sa.Enum('STDIO', 'SSE', 'WEBSOCKET', name='transporttypeenum'), nullable=True),
        sa.Column('tools', sa.JSON(), nullable=True),
        sa.Column('config', sa.JSON(), nullable=True),
        sa.Column('status', sa.Enum('CONNECTED', 'DISCONNECTED', 'ERROR', name='serverstatusenum'), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_table('build_history',
        ...
        sa.ForeignKeyConstraint(['project_id'], ['mcp_projects.id'], ),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('build_id')
    )
    op.create_index(op.f('ix_build_history_build_id'), 'build_history', ['build_id'], unique=True)
    ...
    op.create_table('client_connections',
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(['client_id'], ['llm_clients.id'], ),
        sa.ForeignKeyConstraint(['server_id'], ['mcp_servers.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_client_connections_id'), 'client_connections', ['id'], unique=False)
    ...
    op.create_table('tool_permissions',
        ...
        sa.Column('client_id', sa.Integer(), nullable=False),
        sa.Column('server_id', sa.Integer(), nullable=False),
        sa.Column('permission', sa.Enum('ALLOWED', 'DENIED', 'PENDING', name='permissionstatusenum'), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(['client_id'], ['llm_clients.id'], ),
        sa.ForeignKeyConstraint(['server_id'], ['mcp_servers.id'], ),
